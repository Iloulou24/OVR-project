...
def intialisation_KNN(point_cherche, données):
    ...
    while k > len(dataset_test):
        k = int(
            input(f"nombre pas valide, entrez un nombre valide (Max{len(dataset_test)})"))
...
for d in range(class_max):
    train = entrainement(dico_dataset[(
        'dataset', d)], perceptrons[d]["w"], perceptrons[d]["b"], learning_rate)
    w, b, succès = train[0], train[1], train[2]
    perceptrons[d] = {"w": list(w), "b": b, "success": succès}


point = input(
    "entrez des données de même longueur que celle des données essayées")
...
# teste la précision des perceptron
if meilleur_score <= 0 or delta_score <= 0.2 * abs(liste_score[0]):
    ...
    voisins = trouver_k_plus_proches_voisins(
        point, dataset_test, result_KNN[1])
